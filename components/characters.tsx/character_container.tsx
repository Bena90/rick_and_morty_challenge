...
export async function CharacterContainer ({ title }: Props) {
  const characters: Character[] = await getCharacters({});

  return (
    <div className='flex-1 rounded-[24px] p-3'>
      <h2 className={`${myFont.className} text-4xl text-green mb-4`}>{title}</h2>
      <div className='bg-neutral-950 grid grid-cols-2 w-full gap-6 max-h-[600px] overflow-y-scroll scrollbar p-4 rounded-2xl border border-slate-800'>
        {
